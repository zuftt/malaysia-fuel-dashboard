...
**Production:**
- FastAPI: AWS Lambda (containerized) behind API Gateway
- Next.js: Static export to S3 + CloudFront (with WAF)
- PostgreSQL: AWS RDS
- Scrapers: Lambda functions on EventBridge schedule
- GitHub Actions: CI/CD pipeline (Terraform + ECR + Lambda deploy)
